<template>
    <div>
        <i-button class = 'saveReport' type="default" @click="saveUri">保存</i-button>
        <div class="report">
            ...
            <div id ="smallSizeInput">
                <div class = 'smallSize'>
                    接口名称:<i-Input placeholder="" class = 'reportInput' v-model="uriContent.descrip"></i-Input>
                </div>
                <div class = 'smallSize'>
                    接口路由:<i-Input placeholder="" class = 'reportInput' v-model="uriContent.uri"/>
                </div>
            </div>
        </div>
    </div>
...
export default {
    data(){
        return{
            uriContent:this.$store.uriContent
        }
    },
   methods:{
        reSet:function(){
            this.$data.uriContent.descrip = descrip
            this.$data.uriContent.uri = uri
        },
        saveUri:function(){
            this.axios.post('http://10.1.6.241:9200/daily/uri/updateUri',
            qs.stringify({
                uriId:this.$data.uriContent.uriId,
                descrip:this.$data.uriContent.descrip,
                uri:this.$data.uriContent.uri
            })
            )
            .then(res => {
                console.log(res)
